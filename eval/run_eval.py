...
Outputs:
  eval/results/<timestamp>/scores.json     — per-question scores
  eval/results/<timestamp>/summary.json    — aggregate metrics
  eval/results/<timestamp>/report.txt      — human-readable report

Usage:
  python eval/run_eval.py                           # full eval (retrieval + synthesis + judge)
  python eval/run_eval.py --no-synth                # retrieval-only (skip synthesis & judge)
  python eval/run_eval.py --no-judge                # retrieve + synthesize but skip LLM judge
  python eval/run_eval.py --golden eval/golden_set.json  # custom golden set
  python eval/run_eval.py -t IND-01                 # run single question by ID
  python eval/run_eval.py --category "I."           # run a category
"""
...
def main():
    ...
    out_dir = Path(args.output_dir) if args.output_dir else Path("eval/results") / timestamp
