"""
taxflow_kb/layer4/hierarchical_retriever.py
...
class HierarchicalRetriever:
    ...
    Replaces the former two-class pattern (HierarchicalRetriever wrapping
    ...
    def __init__(
        ...
        """
        from taxflow_kb.layer4.retriever import (
            Layer3Retriever, BM25Retriever,
            Layer1Retriever, Layer2Retriever,
        )

        # ── Leaf retrievers ──────────────────────────────────────────────
        self._l3_vector = Layer3Retriever(
            pg_dsn=pg_dsn, api_key=api_key,
            pool=pool, embedding_client=embedding_client,
        )
        self._l3_bm25 = BM25Retriever(pg_dsn=pg_dsn, pool=pool) if enable_bm25 else None
        self._l1_retriever = Layer1Retriever(pg_dsn=pg_dsn, pool=pool) if enable_layer1 else None
        self._l2_retriever = Layer2Retriever(pg_dsn=pg_dsn, pool=pool) if enable_layer2 else None
    ...
    def retrieve(
        ...
        logger.info(
            "HierarchicalRetriever: %d contexts in %.0f ms "
    ...
    def classify_and_retrieve(
        ...
        pub_filter: Optional[list[str]] = None,
        tax_year: Optional[int] = None,
    ) -> tuple[dict, list[RetrievedContext], float]:
        """
    ...
    def _search(
        ...
        layer1_contexts: list[RetrievedContext] = []
        layer2_contexts: list[RetrievedContext] = []
        l1_ms = 0.0
        l2_ms = 0.0
        ...
        final_contexts: list[RetrievedContext] = []
        if layer1_contexts:
    ...
    def _navigate(
        ...
        """
        from taxflow_kb.layer4.retriever import normalize_query
