"""
taxflow_kb/layer4/retriever.py
...
Contains:
  ...
  - Layer3Retriever          — pgvector cosine similarity
  - BM25Retriever            — Postgres tsvector full-text search
  - Layer1Retriever          — MeF business rules
  - Layer2Retriever          — Instruction graph (form/line refs)

The orchestrator that ties these together is HierarchicalRetriever
(in hierarchical_retriever.py). MultiLayerRetriever is a deprecated
alias that points to HierarchicalRetriever for backward compatibility.
"""
...
def _hybrid_merge_augment(
    vec_contexts     : list[RetrievedContext],
    ...
    def _key(ctx: RetrievedContext) -> str:
...
class Layer3Retriever:
    """
    Semantic search over IRS publication chunks using pgvector cosine similarity.

    Wraps Layer3Store.search_similar() and converts results into
    RetrievedContext objects tagged retrieval_method="vector".
    ...
    def retrieve(
        ...
        # Use connection pool if available, otherwise create new store per query
        if self._pool is not None:
            conn = self._pool.getconn()
            try:
                with Layer3Store(conn=conn) as store:
            ...
            with Layer3Store(dsn=self._pg_dsn) as store:
                raw = store.search_similar(
                    q_vec,
                    top_k       = top_k,
                    pub_numbers = pub_numbers,
...
class BM25Retriever:
    ...
    Wraps Layer3Store.search_bm25() (plainto_tsquery + ts_rank_cd).
...
# ──────────────────────────────────────────────────────────────────────────────
# MultiLayerRetriever has been merged into HierarchicalRetriever.
...
class _MultiLayerRetrieverAlias:
    """Deprecated: use HierarchicalRetriever instead."""
    def __new__(cls, *args, **kwargs):
        import warnings
        warnings.warn(
            "MultiLayerRetriever is deprecated; use HierarchicalRetriever instead.",
