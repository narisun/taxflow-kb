"""
taxflow_kb/ingestion/postgres_ingestion.py
...
class PostgresIngestion:
    ...
    Usage:
        pg = PostgresIngestion("postgresql://user:pass@localhost:5432/taxflow")
        pg.ensure_schema()
    ...
    def ensure_schema(self, schema_file: Optional[str] = None) -> None:
        ...
        Defaults to schema/postgres_schema.sql relative to the project root.
        """
        if schema_file is None:
            schema_file = str(
                Path(__file__).parent.parent.parent / "schema" / "postgres_schema.sql"
            )
    ...
    def save_test_result(
        ...
        Args:
            ...
            tax_year: Tax year for this test result. Defaults to the configured default_tax_year if not provided.
        """
        if tax_year is None:
