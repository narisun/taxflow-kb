...
Usage:
    ...
    python scripts/ingest_all_pubs.py

    # Download only (no DB required)
    python scripts/ingest_all_pubs.py --download-only

    # Ingest only (skip download, assumes PDFs already present)
    python scripts/ingest_all_pubs.py --ingest-only

    # Dry run — show what would be done
    python scripts/ingest_all_pubs.py --dry-run

    # Skip embeddings (text-only, no OpenAI cost)
    python scripts/ingest_all_pubs.py --skip-embedding

    # Ingest specific pubs only
    python scripts/ingest_all_pubs.py --pubs 17 535 560 541

    # Force re-ingest even if already in DB
    python scripts/ingest_all_pubs.py --force
...
def main():
    ...
    parser.add_argument("--pg-dsn", default=os.environ.get("PG_DSN", ""),
                        help="PostgreSQL DSN (default: $PG_DSN)")
