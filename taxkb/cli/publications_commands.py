"""
taxkb/cli/publications_commands.py
...
def cmd_validate_publications(args) -> int:
    ...
    if step in ("all", "v3.2"):
        from taxkb.publications.validation.coverage import validate_coverage
        from taxkb.publications.store import PublicationStore
    ...
    if step in ("all", "v3.3"):
        from taxkb.publications.validation.retrieval import validate_retrieval
        from taxkb.publications.store import PublicationStore
...
def cmd_re_embed(args) -> int:
    ...
    import os
    from taxkb.publications.store  import PublicationStore
    from taxkb.publications.embeddings       import embed_chunks, EMBEDDING_MODEL
...
def cmd_download_publications(args) -> int:
    ...
    from taxkb.rules.irs_download import IRSDownloader
