...
def cmd_ingest(args) -> int:
    ...
    if args.neo4j_uri:
        from taxkb.rules.neo4j_store import RuleGraphStore
    ...
    if args.pg_dsn:
        from taxkb.rules.postgres_store import RuleStore
...
def cmd_validate(args) -> int:
    ...
    if step in ("all", "v1.1"):
        from taxkb.rules.validation import structural as v1_structural
    ...
    if step in ("all", "v1.3"):
        from taxkb.rules.validation import test_replay as v1_test_replay
        from taxkb.rules.validation.test_replay import build_synthetic_test_cases
...
def cmd_sample(args) -> int:
    ...
    from taxkb.rules.validation import spot_check
...
def cmd_diff(args) -> int:
    ...
    from taxkb.rules.validation import regression
    from taxkb.rules.validation.test_replay import build_synthetic_test_cases
    test_cases = build_synthetic_test_cases()
    ...
    _print_report(report)
    from taxkb.models import ChangeType
