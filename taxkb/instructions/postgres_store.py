"""
taxkb/instructions/postgres_store.py
...
from taxkb.instructions.models import Layer2ParseResult
...
class InstructionStore:
    ...
    def ensure_schema(self, sql_path: str | None = None) -> None:
        """Run the Layer 2 DDL file to create tables if they don't exist.

        Defaults to ``taxkb/schema/postgres_layer2.sql``.
        """
        from pathlib import Path
        if sql_path is None:
            sql_path = str(
                Path(__file__).resolve().parent.parent / "schema" / "postgres_layer2.sql"
            )
