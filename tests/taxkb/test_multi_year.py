...
class TestCLICoverageCommand:
    ...
    def test_coverage_command_exists_in_dispatch(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "taxkb", "coverage", "--help"],
    ...
    def test_coverage_command_runs(self):
        ...
        import subprocess, sys
        result = subprocess.run(
            [sys.executable, "-m", "taxkb", "coverage"],
            capture_output=True, text=True,
            cwd=_PROJECT_ROOT,
        )
        assert result.returncode == 0
    ...
    def test_coverage_gaps_only_runs(self):
        ...
        import subprocess, sys
        result = subprocess.run(
            [sys.executable, "-m", "taxkb", "coverage", "--gaps-only"],
            capture_output=True, text=True,
            cwd=_PROJECT_ROOT,
        )
        assert result.returncode == 0
...
class TestDefaultYearBehavior:
    ...
    def test_agent_defaults_to_current_year(self):
        ...
        from taxkb.agent.classifier import QueryMetadata, QueryIntent
        ...
        with patch("taxkb.agent.agent.classify_query", return_value=no_year_meta):
    ...
    def test_agent_uses_detected_year(self):
        ...
        with patch("taxkb.agent.agent.classify_query", return_value=year_2024_meta):
            agent = MagicMock(spec=TaxBrainAgent)
            agent._config = AgentConfig(default_tax_year=2025, enable_compression=False)
            agent.api_key = "sk-test"
            agent._retriever = MagicMock()
    ...
    def test_cli_search_help_does_not_expose_all_years(self):
        ...
        import subprocess, sys
        result = subprocess.run(
            [sys.executable, "-m", "taxkb", "search", "--help"],
            capture_output=True, text=True,
            cwd=_PROJECT_ROOT,
        )
        assert result.returncode == 0
    ...
    def test_cli_ask_help_does_not_expose_all_years(self):
        ...
        import subprocess, sys
        result = subprocess.run(
            [sys.executable, "-m", "taxkb", "ask", "--help"],
            capture_output=True, text=True,
            cwd=_PROJECT_ROOT,
        )
        assert result.returncode == 0
...
class TestCrossYearComparison:
    ...
    def test_extract_comparison_years_from_to(self):
        ...
        from taxkb.agent.classifier import _extract_comparison_years
    ...
    def test_agent_single_year_query_makes_one_call(self):
        ...
        with patch("taxkb.agent.agent.classify_query", return_value=single_meta):
            agent = MagicMock(spec=TaxBrainAgent)
            agent._config = AgentConfig(default_tax_year=2025, enable_compression=False)
            agent.api_key = "sk-test"
            agent._retriever = MagicMock()
