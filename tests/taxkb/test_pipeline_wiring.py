...
from taxkb.publications.chunk_enrichment import (
    EnrichmentRegistry,
    enrich_for_embedding,
)
from taxkb.publications.models import PublicationChunk
from taxkb.publications.ontology import ChunkType
...
class TestIngestWithSummaries:
    ...
    @patch("taxkb.publications.store.PublicationStore.upsert_publication")
    @patch("taxkb.publications.store.PublicationStore.upsert_chunks")
    @patch("taxkb.publications.store.PublicationStore.upsert_embeddings")
    @patch("taxkb.publications.store.PublicationStore.finalize_embedding")
    @patch("taxkb.publications.embeddings.embed_chunks")
    def test_ingest_with_summaries_generates_anchors(
        ...
        from taxkb.publications.summary_generator import generate_anchor_chunks
...
class TestCLIHierarchy:
    ...
    def test_add_hierarchy_columns_command_exists(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "taxkb", "add-hierarchy-columns", "--help"],
    ...
    def test_no_summaries_flag_in_ingest_publications(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "taxkb", "ingest-publications", "--help"],
            capture_output=True, text=True,
            cwd=str(__import__("pathlib").Path(__file__).resolve().parent.parent.parent),
        )
