...
class TestClaudeVisionExtractor:
    @pytest.mark.asyncio
    async def test_w2_extraction(self):
        fields = {
        ...
        result = await extractor.extract(file_bytes, "W-2")
    ...
    async def test_low_confidence_flagged(self):
        ...
        result = await extractor.extract(file_bytes, "1099-INT")
    ...
    @pytest.mark.asyncio
    async def test_image_file_detected(self):
        fields = {"box1_wages": {"value": "50000.00", "confidence": 0.90}}
        extractor = ClaudeVisionExtractor(_mock_client(fields))
        file_bytes = b"\x89PNG\r\n\x1a\n fake image"

        await extractor.extract(file_bytes, "W-2")
        call_args = extractor.client.messages.create.call_args
        content = call_args.kwargs["messages"][0]["content"]
        assert content[0]["type"] == "image"

    @pytest.mark.asyncio
    async def test_pdf_file_detected(self):
        fields = {"box1_wages": {"value": "50000.00", "confidence": 0.90}}
        extractor = ClaudeVisionExtractor(_mock_client(fields))
        file_bytes = b"%PDF-1.4 fake"
    ...
    @pytest.mark.asyncio
    async def test_correct_prompt_selected(self):
        fields = {"payer": {"value": "CHASE", "confidence": 0.95}}
        extractor = ClaudeVisionExtractor(_mock_client(fields))
        file_bytes = b"%PDF-1.4 fake"

        await extractor.extract(file_bytes, "1099-INT")
        call_args = extractor.client.messages.create.call_args
