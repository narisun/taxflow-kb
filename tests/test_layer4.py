...
from taxflow_kb.layer4.validation.v4_1_agent import validate_agent, V41Result
...
class TestCPAQueryResult:
    ...
    def test_sources_deduplicated(self):
        ...
        r   = CPAQueryResult(query="q", answer="a", contexts=[ctx, ctx])
    ...
    def test_sources_multiple_pubs(self):
        ...
        r  = CPAQueryResult(query="q", answer="a", contexts=[c1, c2])
...
class TestCPAQueryAgent:
    ...
    @patch("taxflow_kb.layer4.agent.OpenAICompletionClient")
    @patch("taxflow_kb.layer4.agent.OpenAIEmbeddingClient")
    @patch("taxflow_kb.layer4.agent.PgConnectionPool")
    @patch("taxflow_kb.layer4.hierarchical_retriever.HierarchicalRetriever.__init__", return_value=None)
    def test_init_requires_api_key(self, _mock_retriever, _mock_pool, _mock_emb, _mock_comp):
        import os
        from unittest.mock import PropertyMock
        original = os.environ.pop("OPENAI_API_KEY", None)
        try:
            # Also mock settings so .env file doesn't leak a real key
            mock_settings = MagicMock()
            mock_settings.pg_dsn = "postgresql://x/y"
            ...
            mock_settings.neo4j_password.get_secret_value.return_value = ""
            with patch("taxflow_kb.layer4.agent.get_settings", return_value=mock_settings):
                with pytest.raises(ValueError, match="API key"):
                    CPAQueryAgent(pg_dsn="postgresql://x/y", api_key="")
        finally:
            if original:
    ...
    @patch("taxflow_kb.layer4.agent.PgConnectionPool")
    @patch("taxflow_kb.layer4.hierarchical_retriever.HierarchicalRetriever.__init__", return_value=None)
    def test_empty_question_returns_error(self, mock_ret_init, mock_pool_cls, mock_emb_cls, mock_comp_cls):
        agent = CPAQueryAgent(pg_dsn="postgresql://x/y", api_key="sk-test")
        result = agent.query("   ")
        assert result.error
        assert result.answer == ""

    @patch("taxflow_kb.layer4.synthesizer.synthesize")
    @patch("taxflow_kb.layer4.agent.OpenAICompletionClient")
    @patch("taxflow_kb.layer4.agent.OpenAIEmbeddingClient")
    @patch("taxflow_kb.layer4.agent.PgConnectionPool")
    @patch("taxflow_kb.layer4.hierarchical_retriever.HierarchicalRetriever.__init__", return_value=None)
    def test_query_full_flow(self, mock_ret_init, mock_pool_cls, mock_emb_cls, mock_comp_cls, mock_syn):
        # Set up retriever mock — HierarchicalRetriever.retrieve returns 3-tuple
        ...
        agent._retriever.retrieve.return_value = (contexts, 220.0, {"mode": "hierarchical", "nav_pubs": [], "ontology_pubs": []})
    ...
    @patch("taxflow_kb.layer4.agent.PgConnectionPool")
    @patch("taxflow_kb.layer4.hierarchical_retriever.HierarchicalRetriever.__init__", return_value=None)
    def test_no_contexts_returns_error(self, mock_ret_init, mock_pool_cls, mock_emb_cls, mock_comp_cls, mock_syn):
        agent  = CPAQueryAgent(pg_dsn="postgresql://x/y", api_key="sk-test")
        agent._retriever = MagicMock()
        agent._retriever.retrieve.return_value = ([], 50.0, {"mode": "flat", "nav_pubs": [], "ontology_pubs": []})
    ...
    @patch("taxflow_kb.layer4.agent.PgConnectionPool")
    @patch("taxflow_kb.layer4.hierarchical_retriever.HierarchicalRetriever.__init__", return_value=None)
    def test_synthesize_false_skips_llm(self, mock_ret_init, mock_pool_cls, mock_emb_cls, mock_comp_cls, mock_syn):
        contexts = [_make_context() for _ in range(3)]

        agent  = CPAQueryAgent(pg_dsn="postgresql://x/y", api_key="sk-test")
        agent._retriever = MagicMock()
        agent._retriever.retrieve.return_value = (contexts, 100.0, {"mode": "hierarchical", "nav_pubs": [], "ontology_pubs": []})
    ...
    @patch("taxflow_kb.layer4.synthesizer.synthesize", side_effect=RuntimeError("timeout"))
    @patch("taxflow_kb.layer4.agent.OpenAICompletionClient")
    @patch("taxflow_kb.layer4.agent.OpenAIEmbeddingClient")
    @patch("taxflow_kb.layer4.agent.PgConnectionPool")
    @patch("taxflow_kb.layer4.hierarchical_retriever.HierarchicalRetriever.__init__", return_value=None)
    def test_synthesis_error_returns_partial_result(self, mock_ret_init, mock_pool_cls, mock_emb_cls, mock_comp_cls, _mock_syn):
        contexts = [_make_context() for _ in range(5)]

        agent  = CPAQueryAgent(pg_dsn="postgresql://x/y", api_key="sk-test")
        agent._retriever = MagicMock()
        agent._retriever.retrieve.return_value = (contexts, 200.0, {"mode": "hierarchical", "nav_pubs": [], "ontology_pubs": []})
...
class TestV41Gate:
    ...
    def _make_mock_agent(self, result: CPAQueryResult):
        agent = MagicMock()
