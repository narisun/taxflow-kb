...
interface Form1099IntData {
  payer_name?: string; payer_tin?: string; recipient_name?: string; recipient_tin?: string;
  interest_income?: number; early_withdrawal_penalty?: number; interest_on_savings_bonds?: number;
  federal_tax_withheld?: number; investment_expenses?: number; foreign_tax_paid?: number;
  tax_exempt_interest?: number; [key: string]: unknown;
}
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <div key={item.box} className="border border-divider rounded p-2">
              <div className="text-[10px] text-tertiary mb-0.5">Box {item.box} - {item.label}</div>
